...
run_rcspa <- function (c1_data, c2_data){
	...
	#return(list(TRAIN=trainData, TEST=testData, LABELS=classLabels))	

	for (i in 1:length(G)){
		gamma <- G[i]
		Ws[[i]] <- csp_components(trainData, classLabels, gamma, alpha)
		curTrainX <- rcspa_fe_mat(trainData, Ws[[i]])		
		#return(list(curTrainX = curTrainX, W = W, LABELS=classLabels))	
...
#}
#training_data <- rbind(larm_data[1:2000, 3:16], rarm_data[1:2000, 3:16])
#trainingTargets <- class.ind (c(rep("larm", 2000), rep("rarm", 2000)))
#testing_data <- rbind(larm_data[2001:3000, 3:16], rarm_data[2001:3000, 3:16])
#testingTargets <- class.ind (c(rep("larm", 1000), rep("rarm", 1000)))
#trainingSamps <- sample (1:4000, 4000, replace = FALSE)
#testingSamps <- sample (1:2000, 2000, replace = FALSE)
#training_data <- training_data[trainingSamps,]
#trainingTargets <- trainingTargets[trainingSamps,]
#testing_data <- testing_data[testingSamps,]
#testingTargets <- testingTargets[testingSamps,]
#pcaComps <- pca_components(training_data, dim(training_data)[2])
#trainingFeatures <- training_data
#for (i in 1:2000) trainingFeatures[i,] <- pca_fe(training_data[i,], pcaComps)
#testingFeatures <- testing_data
#for (i in 1:1000) testingFeatures[i,] <- pca_fe(testing_data[i,], pcaComps)
#nn <- nnet(x = trainingFeatures, y = trainingTargets, size = nn_size, rang = nn_rang, decay = nn_decay, maxit = nn_maxit)
#result <- test.cl (testingTargets, predict(nn, testingFeatures))
